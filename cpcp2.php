    ...
    if (is_file($file) && is_readable($file) && is_writable($file)) {
        ...
        <div class="path">
            <div class="edit-file-actions">
                <a title="Back" href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-goback"></i> Back</a>
                <div class="edit-file-actions float-right">
                    <a title="Save" href="#" onclick="document.getElementById('edit-form').submit();return false;"><i class="icon-save"></i> Save</a>
                </div>
            </div>
            <h2>Editing File: <?php echo fm_enc(basename($file)) ?></h2>

            <form method="post" id="edit-form">
                <input type="hidden" name="savedata" value="1">
                <textarea name="content" style="width: 100%; height: 500px; margin: 10px 0; font-family: Consolas, Monaco, monospace;"><?php echo fm_enc($content) ?></textarea>
            </form>
        </div>
        <?php
        fm_show_footer();
...
if (isset($_GET['view'])) {
    ...
    <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;edit=<?php echo urlencode($file) ?>"><i class="icon-edit"></i> Edit</a></b> &nbsp;
    <?php
    // ZIP actions
    if ($is_zip && $filenames !== false) {
...
if ($parent !== false) {
    ?>
<tr><td></td><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><a href="?p=<?php echo urlencode(fm_get_parent_path(FM_PATH)) ?>"><i class="icon-arrow_up"></i> ..</a></td></tr>
<?php
}
foreach ($folders as $f) {
...
<?php endif; ?>
<td>
<a title="Edit" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;edit=<?php echo urlencode($f) ?>"><i class="icon-edit"></i></a>
...
.icon-edit { background-position:-336px 0 }
.icon-save { background-position:-384px 0 }
.edit-file-actions { margin-bottom: 10px; }
.edit-file-actions a { margin-right: 10px; }
